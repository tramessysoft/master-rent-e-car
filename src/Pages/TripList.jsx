...
const TripList = () => {
  ...
  const filteredTrip = trip.filter((dt) => {
    ...
    const matchesSearch =
      ...
      dt.transport_type?.toLowerCase().includes(term) ||
  ...
  const exportExcel = () => {
    ...
    const formattedData = filteredData.map((dt, index) => {
      ...
      return {
        ...
        "পিকআপ পয়েন্ট": dt.load_point,
        "ড্রপ পয়েন্ট": dt.unload_point,
  ...
  const printTable = () => {
    const printRows = filteredTrip
      ...
      <tr>
        ...
        <td>${dt.trip_date}</td>
        <td>
          নামঃ ${dt.driver_name}<br/>
          মোবাইলঃ ${dt.driver_contact}<br/>
          কমিশনঃ ${dt.driver_percentage}
        </td>
        <td>
          পিকআপঃ ${dt.load_point}<br/>
          ড্রপঃ ${dt.unload_point}<br/>
          সময়ঃ ${dt.trip_time}
        </td>
        <td>
          নামঃ ${dt.customer ?? ""}<br/>
